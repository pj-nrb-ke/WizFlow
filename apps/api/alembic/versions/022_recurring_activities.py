"""Add recurring_activities, activity_cycles, activity_obligations tables.
...
def upgrade() -> None:
    op.create_table(
        ...
        # What happens
        sa.Column("completion_mode", sa.String(20), nullable=False, server_default="acknowledge"),
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("company_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("companies.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("activity_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("recurring_activities.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("due_date", sa.Date, nullable=False),
        sa.Column("period_label", sa.String(40), nullable=False, server_default=""),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.UniqueConstraint("activity_id", "due_date", name="uq_activity_cycle_due"),
    )
    ...
    op.create_table(
        ...
        sa.Column("company_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("companies.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("activity_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("recurring_activities.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("cycle_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("activity_cycles.id", ondelete="CASCADE"), nullable=False, index=True),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.UniqueConstraint("cycle_id", "user_id", name="uq_activity_obligation"),
    )


def downgrade() -> None:
    op.drop_table("activity_obligations")
